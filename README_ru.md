...
**Важно:** расширение `--output` определяет формат: `.json` → JSON, иначе YAML. При этом файл записывается **без обёртки `templates:`** (плоский словарь), что упрощает использование как `cells_file`.
