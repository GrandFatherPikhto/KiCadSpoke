# fieldstool — tool for bulk set/rename of Role/Cluster (or any custom
# field) directly in .kicad_sch — not a PCB-side IPC field write
...
#   IPC entirely). The BoardConnection fieldstool/gui/main_window.py is
#   driven through is a deliberate exception, and even that is read-only.
...
# around.
#
# Status 2026-08-02: offline core (set/rename via fieldstool_cli.py) and
# fieldstool/gui/ embedded as a kicadstamp_gui.py dock (gui/docks/
# fieldstool_dock.py) — see techdocs/handoff/handoff_2026_08_01_fieldstool.md.
# A standalone entry point (fieldstool_gui.py, its own BoardConnection and
# polling timers) existed until 2026-08-02, then was retired as pure
# duplication of the embedded tab — fieldstool/gui/main_window.MainWindow now
# always requires an injected connection. Not yet verified against a live
# KiCad session (offscreen/synthetic tests only).
