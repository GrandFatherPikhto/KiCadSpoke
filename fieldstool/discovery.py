...
from .blocks import find_balanced_span

_SHEETFILE_RE = re.compile(r'\(property "Sheetfile" "((?:[^"\\]|\\.)*)"')


def _sheet_files_in(text: str) -> List[str]:
    """Sheetfile values (still escaped, see blocks.escape_sexp_string's
    inverse) referenced by every (sheet ...) block in one file's text, in
    file order."""
    files = []
