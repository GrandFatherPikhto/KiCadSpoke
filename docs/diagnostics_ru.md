...
**no-op транзакция** `begin_commit()` → `update_items([fp])` без изменений → `push_commit()` — ровно тот
путь, что реально использует `adapter.commit_with_retry()` в бою, а не голый `update_items()`. После каждой
ступени снимает «пульс» (`ping` + сверка списка PID `kicad.exe`), чтобы точно увидеть, на каком шаге и в
какой момент процесс умер. Лог пишется построчно с flush — последняя строка честна, даже если KiCad умрёт
мгновенно.
...
`run_action("...InteractiveEdit.flip")`, минуя commit — они не «лечат» уязвимое состояние). Теперь ступень 9
честно оборачивает no-op запись в такую же транзакцию, как в бою, с логом каждого под-вызова отдельно.
