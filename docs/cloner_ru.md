...
Модуль `cloner/` является **автономным** и не зависит от `kicad/adapter.py`, `placement/` или `geometry/`. Он используется только в `kicadstamp_cli.py` через команду `clone-extract`.

Его выходные данные (YAML-снимки) предназначены для **ручного анализа** разработчиком. На основе этих снимков пишутся конфигурации для `ClonePlacement` с указанием параметров `params` и `nets`, а также для создания параметризованных шаблонов с помощью `--net-template` и `--param` при извлечении.
...
5. На основе снимка напишите шаблон (`templates`) в вашем конфигурационном файле и укажите `nets` для каждой роли, используя имена цепей из снимка (с учётом иерархических путей). При необходимости используйте `--net-template` и `--param` для параметризации цепей прямо при извлечении (см. команду `extract`).
