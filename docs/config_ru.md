...
- `vias:` — `offset_along_mm`/`offset_across_mm` (локальные), `net:` (`null`/пусто означает
  ...
  `diameter_mm`. Та же конвенция `null` и у `net:` в `tracks:` ниже. Чекбокс **"Rule net (null)"**
...
через **`rules:`**/ManualSpoke сразу в НЕСКОЛЬКИХ Rule с разными нетами (например, пара
...
`rules:`/ManualSpoke ничего не делает (у `ManualSpoke` вообще нет поля `params:`, против которого
