...
- **По сетям** (повторяющиеся секции — pi-фильтры, DAC-каналы), когда заданы `params`/`nets`: каждая
  ...
  подстановки, что у `net:` у via/треков). Неоднозначные кандидаты (2+ матчащих на резолвленную цепь)
  сужаются каскадом `anchor_sheet` → `Cluster` → текущее выделение на плате → физическая близость к
  якорю → фатал, в этом порядке — точный каскад см. в докстрингах `clone_role_resolver.py`.
- **По выделению** (редкие, одноразовые секции — одна MCU), когда `params`/`nets` отсутствуют (или
  задан явный `by_selection: true`, если `params` присутствует только ради резолва цепей via/треков
  и иначе был бы ошибочно принят за режим «по сетям»): роли резолвятся против того, что сейчас
  выделено на живой плате в PCB-редакторе.
