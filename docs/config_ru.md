...
используется — спицей `rules:` или записью `clone_placements:`.
...
- `vias:` — `offset_along_mm`/`offset_across_mm` (локальные), `net:` (`null`/пусто означает
  «наследовать цепь правила» — так умеет только `rules:`/ManualSpoke; `clone_placements:` падает
  фаталом на via без цепи, поскольку у него нет единой «цепи правила» для фолбэка), `drill_mm`,
  `diameter_mm`.
- `components:` — `role:` (сравнивается с кастомным полем `Role` на плате, **не** refdes — одна и та же
  ...
  (используется только матчингом ролей по сетям у `clone_placements:` — см. ниже; `rules:`/ManualSpoke
  игнорирует его полностью, матчит роли чисто по `(rule.net, Role)`).
...
`ClonePlacement.cell`/`role`, взаимоисключающие), `xy:`, `rotation_deg:`, `mirror:`, `layer:`, `nets:`,
`params:`, `net_overrides:`, `refs:` — без скоупинга параметров, вложенное размещение никогда не
наследует `params`/`nets` от родителя, та же конвенция, что уже есть у `ClonePlacement.params`.
...
## `rules:` — ManualSpoke (радиальная развязка вокруг одной ИС)
...
| `name` | Опционально, для `--only`. По умолчанию — `net` (см. `rule_effective_name`). **Не** механизм группировки — не переиспользуй один `name:` в нескольких правилах, чтобы объединить их для `--only`, используй общий `Cluster` (`anchor_cluster`/`spoke.cluster`). Если два правила резолвятся в одно и то же эффективное имя — фатал при загрузке. |
| `retired` | По умолчанию `false`. `true` = «не существует на плате» — вычищает via/треки этого правила из реестра. Всегда побеждает `--only`/`--cluster`. |
...
| Поле | Смысл |
|---|---|
| `pad` | Обязательно. К какому паду якоря правила крепится эта спица (строка, как в KiCad — `'17'`, не `17`). |
...
| `retired`/`skip` | Тот же смысл, что у `Rule`, но в рамках только этой спицы. |
...
**Роль → реальный компонент — два режима**, определяются наличием `params`/`nets` (по сетям) или их
отсутствием (по выделению):
- **По сетям** (повторяющиеся секции — pi-фильтры, DAC-каналы): каждая роль внутри ячейки резолвится
  против реальной цепи через `nets:` (буквальное `role: net`) и/или `params:` (заполняет
  `{placeholder}` в собственных `net_template:`-полях ячейки, тот же механизм подстановки, что у
  `net:` у via/треков). Неоднозначные кандидаты (2+ матчащих на резолвленную цепь) сужаются каскадом
  `anchor_sheet` → `Cluster` → текущее выделение на плате → физическая близость к якорю → фатал, в этом
  порядке — точный каскад см. в докстрингах `clone_role_resolver.py`.
- **По выделению** (редкие, одноразовые секции — одна MCU): без `params`/`nets` вовсе (или явный
  `by_selection: true`, если `params` присутствует только ради резолва цепей via/треков и иначе был бы
  ошибочно принят за режим «по сетям») — роли резолвятся против того, что сейчас выделено на живой
  плате в PCB-редакторе.
...
| `nets` | `{role: буквальная_цепь}` — маппинг ролей по сетям. |
...
| `net_overrides` | `{резолвленная_цепь: замена}` — финальная строковая подмена после остального резолва цепей, для случаев, которые система плейсхолдеров не может выразить напрямую. |
...
| `retired` / `skip` | Тот же принцип, что у `Rule` — см. выше. `retired` всегда побеждает `--only`/`--cluster`. |
...
Затем на неё ссылаются по имени из `Rule`/`ClonePlacement`/`ThermalViaArrayConfig`:
...
| `anchor_ref` / `anchor_role`(+`anchor_sheet`+`anchor_cluster`) **или** `anchor_pad` | Живой якорь на плате, те же поля/резолв, что у `Rule`/`ClonePlacement`. |
...
| `xy` | Буквальная, абсолютная координата платы — вообще без живого якоря. |
| `shift_x_mm`/`shift_y_mm` | Сдвиг в мм в абсолютных координатах платы поверх любой из трёх баз выше (не поверх `xy:` — фатал, если заданы оба, просто отредактируй буквальную координату). |

Ровно одно из `{anchor_ref или anchor_role, anchor_point, xy}` должно быть базой — иначе фатал при
загрузке. Собственный `anchor_point:` у `Rule`/`ThermalViaArrayConfig` требует, чтобы указанная точка
резолвилась в настоящий футпринт (без сдвига, не `xy:`-литерал, и не через цепочку, где есть сдвиг) —
им нужно найти конкретный пад по номеру именно на нём; `ClonePlacement` нужна только координата, так
что там подходит любая `Point`.
...
`origin_by_via_net`, `origin_by_component_role`, `origin_by_component_pad` — неизвестные ключи фатальны
(опечатанный ключ, например `origin-by-via-net` с неверным разделителем, раньше тихо игнорировался).
`clone_profiles:` (для `clone-extract`) принимает: `net`, `pcb`, `channel`, `output`.
...
- [docs/python_ru.md](python_ru.md) — сборка тех же объектов `Rule`/`ClonePlacement`/`Cell` из Python
  вместо ручного YAML.
