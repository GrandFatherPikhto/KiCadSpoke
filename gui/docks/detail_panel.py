...
становятся контекстными (общая область формы)"). Used to be three separate
QDockWidgets tabified together; ExtractDock/PlacerDock/RootMetadataDock are
now plain QWidgets (see each module's own docstring) living as pages of one
QStackedWidget here, switched by a QTabBar that drives the stack directly
(the standard "tabs without their own page-widgets" Qt pattern — a
QTabWidget would insist on owning/parenting the pages itself).
...
profile_picked -> show_extract(), file_selected (fires on EVERY click,
including leaf clicks, always BEFORE the more specific signal — see
config_tree.py's _on_clicked) -> show_root() as the fallback for a plain
file/category click; the more specific signal's handler (if any) then
runs right after and wins.
"""
...
_EXTRACT, _PLACER, _ROOT = range(3)
