...
What it does (read-only, never mutates the board):
  ...
  5. Writes boards/3ch-awg-tia/profiles/templates/p3v3_ldo_composite.yaml
     (a Cell definition, {'cells': {name: {clone_placements: [...]}}} shape
     — wrapped since cells_file:/cell_files: were folded into include: on
     2026-08-02, see handoff_2026_08_02_cells_include_unification.md —
     same format as any other file listed in include:).
...
rewrite p3v3_ldo.yaml by hand to a single clone_placement:
    - name: 3v3_ldo_system
      ...
      xy: [-50.0, 35.0]
and add "templates/p3v3_ldo_composite.yaml" to power.yaml's include:.
...
def main() -> None:
    ...
    print()
    print("Next: add \"templates/p3v3_ldo_composite.yaml\" to power.yaml's include:, "
