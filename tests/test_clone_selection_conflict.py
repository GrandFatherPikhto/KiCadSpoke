...
class TestCloneUsesSelectionMode:
    def test_no_nets_no_params_is_selection_mode(self):
        c = ClonePlacement(name="a", cell="t", origin_x_mm=0, origin_y_mm=0)
        assert clone_uses_selection_mode(c) is True

    def test_nets_present_is_not_selection_mode(self):
        c = ClonePlacement(name="a", cell="t", origin_x_mm=0, origin_y_mm=0, nets={"X": "GND"})
        assert clone_uses_selection_mode(c) is False

    def test_params_present_is_not_selection_mode(self):
        c = ClonePlacement(name="a", cell="t", origin_x_mm=0, origin_y_mm=0, params={"channel": 1})
        assert clone_uses_selection_mode(c) is False
    ...
    def test_by_selection_false_with_empty_nets_and_params_implicit_selection(self):
        ...
        c = ClonePlacement(name="a", cell="t", origin_x_mm=0, origin_y_mm=0, by_selection=False)
        assert clone_uses_selection_mode(c) is True
    ...
    def test_anchor_role_does_not_affect_selection_mode(self):
        ...
        c2 = ClonePlacement(name="b", cell="t", origin_x_mm=0, origin_y_mm=0,
...
class TestCheckSingleSelectionBasedClone:
    def test_single_selection_based_passes(self):
        cfg = _cfg([ClonePlacement(name="a", cell="t", origin_x_mm=0, origin_y_mm=0)])
    ...
    def test_selection_and_nets_based_do_not_conflict(self):
        cfg = _cfg([
            ClonePlacement(name="a", cell="t", origin_x_mm=0, origin_y_mm=0),
            ClonePlacement(name="b", cell="t", origin_x_mm=0, origin_y_mm=0, nets={"X": "GND"}),
        ])
        check_single_selection_based_clone(cfg)
    ...
    def test_retired_clone_not_counted(self):
        cfg = _cfg([
            ClonePlacement(name="a", cell="t", origin_x_mm=0, origin_y_mm=0),
            ClonePlacement(name="b", cell="t", origin_x_mm=0, origin_y_mm=0, retired=True),
        ])
        check_single_selection_based_clone(cfg)
    ...
    def test_three_selection_based_still_fatal(self):
        cfg = _cfg([
            ClonePlacement(name="a", cell="t", origin_x_mm=0, origin_y_mm=0),
            ClonePlacement(name="b", cell="t", origin_x_mm=0, origin_y_mm=0),
            ClonePlacement(name="c", cell="t", origin_x_mm=0, origin_y_mm=0),
        ])
    ...
    def test_by_selection_true_with_nets_counts_as_selection_based(self):
        ...
        cfg = _cfg([
            ClonePlacement(name="a", cell="t", origin_x_mm=0, origin_y_mm=0, nets={"X": "GND"}, by_selection=True),
            ClonePlacement(name="b", cell="t", origin_x_mm=0, origin_y_mm=0, nets={"Y": "GND"}),
        ])
        # a is selection‑based (by_selection), b is nets‑based => no conflict
        check_single_selection_based_clone(cfg)

        cfg2 = _cfg([
            ClonePlacement(name="a", cell="t", origin_x_mm=0, origin_y_mm=0, by_selection=True),
            ClonePlacement(name="b", cell="t", origin_x_mm=0, origin_y_mm=0, by_selection=True),
        ])
...
class TestCheckCloneCellsExist:
    ...
    def test_missing_cell_raises(self):
        cfg = _cfg([ClonePlacement(name="a", cell="does_not_exist", origin_x_mm=0, origin_y_mm=0)])
