...
template layer), same as tracks already did (spoke_layout._resolve_track).
Found live: templates/fpga_cap_pair_spoke.yaml's layer: B.Cu was honoured
...
target_layer), regardless of what the template declared."""
import sys
...
def _run(template, cfg_layer="F.Cu"):
    ...
    rule = Rule(net="NET1", anchor_ref="IC1", spokes=[ManualSpoke(pad="1", template="tpl")])
    cfg = Config(layer=cfg_layer, templates={"tpl": template}, rules=[rule])
...
def test_component_layer_inherits_template_layer_when_slot_has_no_override():
    ...
    placed = _run(template, cfg_layer="F.Cu")
...
def test_component_slot_layer_overrides_template_layer():
    ...
    placed = _run(template, cfg_layer="B.Cu")
